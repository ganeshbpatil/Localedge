...
export class GroqProvider implements IAIProvider {
  ...
  private getClient(options: AIProviderOptions): Groq {
    return new Groq({ apiKey: options.apiKey });
  }

  async complete(request: AICompletionRequest, options: AIProviderOptions): Promise<AICompletionResponse> {
    const client = this.getClient(options);
    const model = request.model ?? options.defaultModel ?? 'llama-3.3-70b-versatile';
